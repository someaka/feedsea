...
// import { articlesStore, embeddingsStore, graphDataStore } from '../stores/stores';
// import type { ArticleType as Article, GraphData } from '$lib/types';


// export const processedArticlesStore = derived(articlesStore, ($articlesStore, set) => {
//     // Get all articles that haven't been processed yet
//     const newArticles = Object.entries($articlesStore).flatMap(([feedId, articles]) =>
//         articles.filter(article => !processedArticles.has(article.id))
//     );

//     // Process new articles
//     if (newArticles.length > 0) {
//         processArticles(newArticles).then(() => {
//             set({ status: 'success', message: 'Embeddings processed successfully.' });
//         }).catch(error => {
//             console.error('Error processing embeddings:', error);
//             set({ status: 'error', message: 'Failed to process embeddings.' });
//         });
//     }
// });
...
//     try {
//         const newEmbeddings = await getEmbeddingsForArticles(articles);
...
//     // Logic to get embeddings for articles
//     // ...
// }

// export const similarityScoresStore = derived(embeddingsStore, ($embeddingsStore, set) => {
//     // Calculate similarity scores based on new embeddings
//     calculateSimilarityScores($embeddingsStore).then(() => {
//         set({ status: 'success', message: 'Similarity scores calculated and graph updated.' });
//     }).catch(error => {
//         console.error('Error calculating similarity scores:', error);
//         set({ status: 'error', message: 'Failed to calculate similarity scores.' });
//     });
// });

// // Function to calculate similarity scores based on embeddings
// async function calculateSimilarityScores(embeddings: Record<string, number[]>) {
//     const newSimilarityScores = await calculateSimilarityScores(embeddings);
//     // Update the graph with new similarity scores
//     updateGraphWithSimilarityScores(newSimilarityScores);
// }

// function updateGraphWithSimilarityScores(similarityScores: Record<string, number>) {
//     // Logic to update the graph using Sigma commands
//     // ...
// }

// function updateGraph(graphData: GraphData) {
//     // Send commands to Sigma to update the live graph
//     // ...
// }