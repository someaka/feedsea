...
class SigmaGrapUpdate {
    ...
    removeNodesById(
        ...
        const tempGraph: Graph = this.graph.copy();
        for (const node of nodes)
            tempGraph.dropNode(node);
        this.graph.import(tempGraph, true);
        // this.addNewLinks(links);
        // this.startLayout();
    }

    addNewNodes(nodes: Node[]) {
        const tempGraph: Graph | null = this.graph.copy();
        for (const node of nodes) {
            tempGraph.addNode(
                node.id,
    ...
    voodoo(tempGraph: Graph) {
        this.graph.import(tempGraph, true);

        // this.stopLayout();
        // this.layout.kill();
        // this.renderer.clear;
        // this.renderer.setGraph(tempGraph);
    ...
    addBoth(graphData: GraphData) {
        if (graphData.nodes.length > 0) this.addNewNodes(graphData.nodes);
        if (graphData.links.length > 0) this.addNewLinks(graphData.links);
    }

    addAll(graphData: GraphData) {
        this.graph.clearEdges();
        this.addBoth(graphData);
    }

    aliveRenderer(graph: Graph) {
        this.renderer = new Sigma(graph, this.container as HTMLDivElement, {
            renderLabels: true,
            labelDensity: 1,
            labelGridCellSize: 150
        });
        if (this.DayOrNight) {
            this.renderer.setSetting("labelColor", { color: '#000000' });
            this.renderer.setSetting("defaultDrawNodeHover", lightDrawDiscNodeHover);
        } else {
            this.renderer.setSetting("labelColor", { color: '#FFFFFF' });
            this.renderer.setSetting("defaultDrawNodeHover", darkDrawDiscNodeHover);
        }
        this.initializeInteractions();
        this.layout = this.setLayout(this.layoutType === "forceAtlas");
        this.startLayout();
...
const removeNodesById = (
    ...
    SigmaGrapUpdate.getInstance()?.removeNodesById(
        nodes,
        // links
        // graphData
    );
